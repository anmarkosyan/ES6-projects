...
console.log(dogSarah); //{weight: 13, curFood: 275, owners: Array(2), recFood: 192}
console.log(`Sarah's dog is eating ${dogSarah.curFood > dogSarah.recFood ? 'too much.' : 'a little.'}`);

//N3
const ownersEatTooMuch = dogs.filter(el => el.curFood > el.recFood).flatMap(el => el.owners);
const ownersEatTooLittle = dogs.filter(el => el.curFood < el.recFood).flatMap(el => el.owners);
...
//N7
const normalEatingDog = dogs.filter(dog => dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1.1);
console.log(normalEatingDog);
